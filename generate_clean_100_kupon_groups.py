...
EXCLUDE_WORDS = [
    ...
    # Yabancı / İlgisiz
    "robotics", "crypto only", "cpa_lenta", "southeast asia", "workflow lab", "chinese",
    "doktora platformu", "yüksek lisans", "yazılımcı gençler", "adana is ilani", "letgo", "izmir", "proxy886",
    # Trendyol Koleksiyon
...
async def build_top_100():
    ...
    print(f"[*] Taranacak toplam aday: {len(candidates)} grup")
                            ...
                            if is_group:
                                ...
                                if m_cnt >= 40:
                                    ...
                                    if len(matched_signals) >= 1:
                                        ...
                                        verified_results.append(rec)
                                        print(f"[{len(verified_results):03d}] 🎟️ ONAYLANDI: @{u:22s} | {title[:28]} | {m_cnt:5d} üye")
                except Exception:
                    pass
