...
try:
    with open("new_10_target_groups.json", "r", encoding="utf-8") as f:
        found_groups = {item["username"].lower(): item for item in json.load(f)}
except Exception:
    found_groups = {}

queries = [
    "hesap satış", "hesap alım satım", "lisans satış", "dijital market",
    "dijital ürün", "canva lisans", "yazılım satış", "hediye çeki alım",
    "indirim çeki alım", "çek pazarı", "çek satım", "fırsat pazarı",
    "kod borsası", "kod merkezi", "kod marketi", "epin satış",
    "epin alım", "oyun kodu satış", "premium hesap satış"
]
...
AD_WORDS = ["satılır", "satilir", "alınır", "alinir", "fiyat", "stok", "dm", "tl", "hesap", "lisans", "kod", "çek", "cek", "canva", "adobe", "chatgpt", "netflix", "premium"]
...
async def main():
    ...
    print(f"\nToplam {len(sorted_groups)} adet kaliteli hedef grup toplandı.")
