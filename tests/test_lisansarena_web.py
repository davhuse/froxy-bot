...
class LisansArenaWebTests(unittest.TestCase):
    ...
    def test_store_health_reports_product_count(self):
        ...
        expected_count = len(self.store.storefront_catalog())
        self.assertEqual(response.get_json()["product_count"], expected_count)
        self.assertEqual(response.get_json()["storefront_count"], expected_count)
        self.assertEqual(response.get_json()["duplicate_records_hidden"], 0)
    ...
    def test_old_render_internal_database_host_uses_regional_external_dns(self):
        with patch.dict(os.environ, {"RENDER": "true", "RENDER_REGION": "oregon"}):
            normalized = store_module.normalize_database_url(
                "postgresql://user:pass@dpg-example123-a/store"
            )
        self.assertEqual(
            normalized,
            "postgresql+psycopg://user:pass@dpg-example123-a.oregon-postgres.render.com/store",
        )
    ...
    def test_telegram_auth_establishes_customer_session(self):
        ...
        self.assertEqual(len(catalog.get_json()["products"]), len(self.store.storefront_catalog()))
