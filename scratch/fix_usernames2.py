...
accounts = [
    {
        ...
        "candidates": ["KeyVadiSatis", "KeyVadi_Satis", "KeyVadiSatisBot", "KeyVadiShop"],
    },
    {
        "phone": "+13412648927",
        ...
        "candidates": ["LisansArenaSatis", "LisansArena_Satis", "LisansArenaShop", "LArenaSatis"],
    },
]

async def set_username(acc):
    phone = acc["phone"]
    client = TelegramClient(StringSession(acc["session"]), api_id, api_hash)
    await client.connect()
    me = await client.get_me()
    print(f"[{phone}] Mevcut username: @{me.username}")
    for u in acc["candidates"]:
        try:
            await client(UpdateUsernameRequest(username=u))
            print(f"[{phone}] [OK] @{u} ayarlandi!")
            break
        except Exception as e:
            print(f"[{phone}] @{u} hata: {e}")
    await client.disconnect()

async def main():
    for acc in accounts:
        await set_username(acc)
        print("-" * 50)
