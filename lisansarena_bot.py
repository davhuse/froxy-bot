...
async def admin_la_kullanici_handler(event):
    ...
    if not target_user:
        await event.respond(f"❌ `{query}` kimliğine sahip kullanıcı LisansArena veritabanında bulunamadı.")
        return
    ...
    resp = (
        f"👤 **KULLANICI DETAYI (LisansArena)**\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🆔 **ID:** `{matched_uid}`\n"
        f"👤 **İsim:** {full_name}\n"
...
async def admin_la_siparisler_handler(event):
    ...
    lines = ["📦 **SON LİSANSARENA SİPARİŞLERİ (Son 10 İşlem)**\n━━━━━━━━━━━━━━━━━━━━"]
